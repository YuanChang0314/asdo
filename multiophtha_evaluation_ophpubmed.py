...
corpus_names_customized = ["PubMed_Ophthalmology", "PubMed_Ophthalmology+statpearls+textbooks"]

with open('../data/results_oph/multi_ophtha.json', 'r') as file:
    data = json.load(file)

for model_name_ in model_names:
    output_name_ = "multiophtha+" + model_name_ + "+CoT"
    print('---', output_name_, '---')
...
### Run baseline situation
for model_name_ in model_names:
    for retriever_name_ in retriever_names:
        for corpus_name_ in corpus_names+corpus_names_customized:
            output_name_ = "multiophtha+" + model_name_ + "+" + retriever_name_ + "+" + corpus_name_
            print('---', output_name_, '---')
            cot = MedRAG(llm_name=model_name_, rag=True, retriever_name=retriever_name_, corpus_name=corpus_name_, rationale_query=False)
            for i, question in tqdm(enumerate(data)):
                if output_name_ in data[i].keys():    
                    continue
                else:
            ...
            with open('../data/results_oph/multi_ophtha.json', 'w') as file:
                json.dump(data, file)


### Run with rationale query
for model_name_ in model_names:
    for retriever_name_ in retriever_names:
        for corpus_name_ in corpus_names_customized:
            output_name_ = "multiophtha+[rationale_query]+" + model_name_ + "+" + retriever_name_ + "+" + corpus_name_
            print('---', output_name_, '---')
            cot = MedRAG(llm_name=model_name_, rag=True, retriever_name=retriever_name_, corpus_name=corpus_name_, rationale_query=True)
            for i, question in tqdm(enumerate(data)):
                if output_name_ in data[i].keys():    
                    continue
                else:
