...
# corpus_names = ["Textbooks", "StatPearls", "PubMed"]

with open('../data/results/BCSC_result.json', 'r') as file:
    data = json.load(file)

# Test without RAG
for model_name_ in model_names:
    output_name_ = "BCSC+" + model_name_ + "+CoT"
    print('---', output_name_, '---')
    ...
    for i, question in tqdm(enumerate(data)):
        ...
        with open('../data/results/BCSC_result.json', 'w') as file:
                json.dump(data, file)
