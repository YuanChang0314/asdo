...
with open('../data/results/MedMCQA_ophthalmology_val_result.json', 'r') as file:
    data = json.load(file)
...
for model_name_ in model_names:
    output_name_ = "MedMCQA(Ophthalmology)+" + model_name_ + "+CoT"
    print('---', output_name_, '---')
    ...
    for i, question in tqdm(enumerate(data)):
        ...
        with open('../data/results/MedMCQA_ophthalmology_val_result.json', 'w') as file:
                json.dump(data, file)
    sys.exit() # Comment this for formal version


for model_name_ in model_names:
    for retriever_name_ in retriever_names:
        for corpus_name_ in corpus_names:
            output_name_ = "MedMCQA(Ophthalmology)+" + model_name_ + "+" + retriever_name_ + "+" + corpus_name_
            print('---', output_name_, '---')
